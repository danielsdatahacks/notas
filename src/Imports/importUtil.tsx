...
export const updateHashtagsFromNoteText = (noteID: string, noteText: string, graph: Graph): Graph => {

    let tempGraph: Graph = graph;
    ...
    if(regExHashtagMatch !== null){
      for(let i in regExHashtagMatch){
        ...
        if(hashtagIsNew){
          let newHashtagID: string = uuidv4();
    ...
    previousHashtagsOfNoteTemp.forEach(function(hashtagID) {
        ...
        if(hashtagID in hashtagDictTemp){
            ...
            if(hashtagDictTemp[hashtagID].LinksTowards.length === 0) {
                tempGraph.DeletedNodeIDs.push(hashtagID);
                delete hashtagDictTemp[hashtagID];
            }
        }
    });

    //Update hashtags of note if it exists already. In the import it may be computed later.
    ...
    tempGraph.TopicDictionary = hashtagDictTemp;

    return tempGraph;
}

export const getHashtagsOfNoteAndUpdatedGlobalHashtagDict = (noteID: string, noteText: string, graph: Graph): NodeHashtags => {
    
    let nodeHashtags: NodeHashtags = {Graph: graph, Hashtags: [] as string[]};

    let hashtagDictTemp = graph.TopicDictionary; //Current hashtags.
    let previousHashtagsOfNoteTemp = [] as string[];
    if(noteID in graph.NodeDictionary){
        previousHashtagsOfNoteTemp = graph.NodeDictionary[noteID].Hashtags;
    }
    let hashtagsOfCurrentNote: string[] = [];
    let regExHashtagMatch: RegExpMatchArray | null = noteText.match(regex.hashtagRegEx);
    if(regExHashtagMatch !== null){
    ...
    previousHashtagsOfNoteTemp.forEach(function(hashtagID) {
        ...
        if(hashtagID in hashtagDictTemp){
            ...
            //If the hashtag has no note anymore the hashtag can be deleted entirely.
            if(hashtagDictTemp[hashtagID].LinksTowards.length === 0) {
                nodeHashtags.Graph.DeletedNodeIDs.push(hashtagID);
                delete hashtagDictTemp[hashtagID];
            }
        }
    });

    nodeHashtags.Graph.TopicDictionary = hashtagDictTemp;
...
// export const updateGraphFromHashtagsOfNote = (noteID: string, hashtagsOfNote: string[], graph: Graph): Graph => {
...
//                 [noteID]: {
//                   ...tempGraph.NodeDictionary[noteID],
//                   Hashtags: hashtagsOfNote
//                 }
//             },
//             TopicDictionary: nodeHashtags.Graph.TopicDictionary
//         };
//     }
